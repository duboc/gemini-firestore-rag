...
# Configuration
class Config:
    PROJECT_ID: str = "conventodapenha"
...
def store_in_firestore(chunks: List[str], embeddings: List[List[float]]) -> None:
    ...
    if not collection.get():
        logger.info(f"Collection '{Config.COLLECTION_NAME}' does not exist. Creating and populating it.")
        batch = db.batch()
        ...
        logger.info(f"Collection '{Config.COLLECTION_NAME}' already exists. Skipping population.")
...
def calculate_firestore_cost(reads: int, writes: int, storage_gb: float) -> Dict[str, float]:
    read_cost = max(0, reads - CostConfig.FIRESTORE_FREE_READS) * CostConfig.FIRESTORE_READ_COST
    write_cost = max(0, writes - CostConfig.FIRESTORE_FREE_WRITES) * CostConfig.FIRESTORE_WRITE_COST
...
def calculate_vertex_cost(input_chars: int, output_chars: int, embedding_chars: int) -> Dict[str, float]:
...
def estimate_costs(repo_size: float, num_files: int, num_queries: int) -> Dict[str, Any]:
    ...
    firestore_costs = calculate_firestore_cost(estimated_reads, estimated_writes, estimated_storage)
...
def process_repository(repo_type: str, repo_path: str, repo_url: str) -> None:
    ...
    st.success(f"Repository processed and stored {len(all_chunks)} chunks in Firestore!")
...
def generate_code_sequence(initial_query: str, num_iterations: int) -> None:
    with st.spinner("Generating code sequence..."):
        ...
        for i in range(num_iterations):
            ...
            code = generate_code(chat, st.session_state.queries[-1], "\n\n".join(st.session_state.rag_references[-1]))
    ...
    st.success("Code generation sequence complete! Switch to the 'Generated Code' tab to view the results.")

def display_generated_code():
    st.subheader("Generated Code")
    if 'generated_code' in st.session_state:
        for i, (code, query, refs) in enumerate(zip(st.session_state.generated_code, st.session_state.queries, st.session_state.rag_references), 1):
...
def display_costs():
    ...
    repo_size = st.number_input("Estimated repository size (in MB)", min_value=1, value=100)
    num_files = st.number_input("Estimated number of Python files", min_value=1, value=50)
    num_queries = st.number_input("Estimated number of queries", min_value=1, value=10)

    if st.button("Estimate Costs"):
        costs = estimate_costs(repo_size / 1024, num_files, num_queries)  # Convert MB to GB
...
def main():
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["Repository Processing", "Code Generation", "Generated Code", "Cost Estimation", "Log"])

    with tab1:
        repo_type = st.radio("Select repository type:", ("Local", "Remote"))
        repo_path = st.text_input("Enter the path to your local repository:", value="repo") if repo_type == "Local" else None
        repo_url = st.text_input("Enter the URL of the GitHub repository:") if repo_type == "Remote" else None
    ...
    with tab2:
        ...
        num_iterations = st.slider("Number of code generations:", min_value=1, max_value=5, value=3)
